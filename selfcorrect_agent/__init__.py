...
Quick start (in your own code):
    ...
    improve_file("path/to/file.py", provider=my_provider)  # optional adapter
...
def run_headless(file_path: str, config: Optional[Config] = None, provider=None) -> Session:
    """Run the self-correction cycle and return the Session without any UI."""
    return SelfCorrectingAgent(config or Config(), provider=provider).run(file_path)


def improve_file(file_path: str, config: Optional[Config] = None,
                 open_browser: bool = True, provider=None) -> None:
    ...
    app = create_app(path, config, provider=provider)
